...
class workspace {
  constructor () {
    ...
    this.notificationButton = get('#notificationButton');
    this.notificationButton.onclick = () => {this.share()};
...
workspace.prototype.saveXML = function (uid) {
  ...
  let data = "data:x-application/xml;charset=utf-8," + encodeURIComponent(xmlText);
	...
	element.setAttribute('download', Tool.makeAName(Code.generateCode(), 'xml')),
...
workspace.prototype.share = function (uid) {
  let xmlText = '';
  if (uid == undefined) {
    xmlText = Blockly.Xml.domToPrettyText(Blockly.Xml.workspaceToDom(Code.workspace));
    xmlText = this.writeWorkspace (xmlText, true);

...
class Tool {
  ...
  */
  static makeAName (code, ext) {
    ...
    if (ext == '') {
      ...
      return desc ? `${desc [1].replaceAll(' ', '_').replaceAll('.', '').slice().substring(0,30)}.${ext}` : imp.length ? `main.${ext}` : `my_BIPES_project.${ext}`;
    }
  }
  /**Converts RGB to HEX
  ...
  */
  ...
  static emptyXML () {
    ...
    // return `<xml xmlns="https://bipes.net.br">
    // <block type="project_metadata" id="" x="-300" y="100">
    ...
    //    <shadow type="text" id="">
    //      <field name="TEXT">Program_Name</field>
    //    </shadow>
    //  </value>
    // </block>
    // <block type="roop" id="" x="-300" y="-300"></block>
    // </xml>`
    return `<xml><block type="roop" id="" x="-300" y="-300"></block></xml>`
...
class files {
  ...
  internalXML () {
    ...
    Tool.updateSourceCode(new Blob([Code.generateXML()], {type: "text/plain"}),  Tool.makeAName(Code.generateCode(), 'xml'));
  }
  /**
  ...
  handleCurrentProject () {
    ...
    this.blocks2Code.XML.innerHTML = Tool.makeAName(Code.generateCode(), 'xml') + '<span>internal</span>'
